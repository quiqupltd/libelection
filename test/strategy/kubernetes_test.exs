defmodule Election.Strategy.KubernetesTest do
  ...
  describe "leader/2 without connected nodes" do
    ...
    test "when fetching nodes fails with a 500 error, returns nil", %{subject: subject} do
      ...
      log =
        capture_log(fn ->
          assert subject.(put_in(@config[:kubernetes_api_base_url], kubernetes_base_url(bypass))) ==
                   nil
        end)
    ...
    test "when other nodes are discovered but not included in the connected nodes, returns nil",
         %{subject: subject} do
      bypass = Bypass.open()
  ...
  describe "leader/2 with connected nodes" do
    ...
    test "when other nodes are discovered, returns the node with the lowest resourceVersion", %{
      subject: subject
    } do
      bypass = Bypass.open()
  ...
  defp fixture(:kubernetes_api_response) do
    %{
      ...
      "items" => [
        %{
          "metadata" => %{
            "creationTimestamp" => "2018-01-11T13:03:10Z",
      ...
      "kind" => "EndpointsList",
      "metadata" => %{
        "resourceVersion" => "5143116",
        "selfLink" => "/api/v1/namespaces/default/endpoints"
      }
    }
  end

  defp fixture(:kubernetes_api_response_no_resource_version) do
    %{
      "apiVersion" => "v1",
      "items" => [
        %{
